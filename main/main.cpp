...
extern "C" void app_main(void)
{
    ...
    ESP_UTILS_CHECK_ERROR_EXIT(SettingsService::instance().begin(), "Init settings service failed");
    ESP_UTILS_CHECK_ERROR_EXIT(SettingsService::instance().ensureWifiStarted(), "Start Wi-Fi service failed");
...

    {
        ...
        lv_timer_create([](lv_timer_t *t) {
            ...
            StatusBar::WifiState wifi_icon_state = StatusBar::WifiState::DISCONNECTED;
            switch (settings.getWifiSignalLevel()) {
            case 3:
                wifi_icon_state = StatusBar::WifiState::SIGNAL_3;
                break;
            case 2:
                wifi_icon_state = StatusBar::WifiState::SIGNAL_2;
                break;
            case 1:
                wifi_icon_state = StatusBar::WifiState::SIGNAL_1;
                break;
            default:
                wifi_icon_state = StatusBar::WifiState::DISCONNECTED;
                break;
            }
            ESP_UTILS_CHECK_FALSE_EXIT(status_bar->setWifiIconState(wifi_icon_state), "Refresh Wi-Fi failed");

            storybook::PowerStatus power = settings.getPowerStatus();
            if (power.available && power.battery_present && (power.battery_percent >= 0)) {
                ESP_UTILS_CHECK_FALSE_EXIT(status_bar->showBatteryIcon(), "Show battery icon failed");
                ESP_UTILS_CHECK_FALSE_EXIT(status_bar->setBatteryPercent(power.charging, power.battery_percent),
                                           "Refresh battery failed");
            } else {
                ESP_UTILS_CHECK_FALSE_EXIT(status_bar->hideBatteryIcon(), "Hide battery icon failed");
            }
        }, 1000, phone);
    }
